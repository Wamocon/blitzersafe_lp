/* ==============================================
   BlitzerSafe Landing Page : Immersive Interactions v3
...
(function () {
  ...
  /* ==============================================
     ...
     with BlitzerSafe text formation on load (looping)
     ============================================== */
  function initParticles() {
    ...
    /* BlitzerSafe text formation state – full cycle:
    ...
    function resize() {
      ...
      /* Re-centre BlitzerSafe text targets whenever canvas dimensions change */
      if (particles.length > 0) assignTargets();
    }

    /* Sample pixel positions from rendered text "BS" – outline only */
    function sampleTextPositions() {
      ...
      /* Position: use visible viewport centre so BlitzerSafe initials are always centred on screen */
    ...
    function animate() {
      ...
      for (var i = 0; i < particles.length; i++) {
        ...
        /* BlitzerSafe text formation force */
  ...
  function initTyping() {
    ...
    var words = ['Bußgeldbescheid', 'PTB-Prüfung', 'Widerspruch'];
    var wordIdx = 0;
    var charIdx = 0;
  ...
  function initSolarSystem() {
    ...
    var stars = [];
    var PLANETS = [
      { abbr: 'BS', orbitR0: 100, r0: 30, speed: 0.00035, angle: Math.PI * 0.25, active: true, _sx: 0, _sy: 0 },
    ...
    function frame(ts) {
      ...
      ctx.fillText('BlitzerSafe', cx, cy);
      ctx.shadowBlur = 0;
  ...
  window.initGalaxy = function () {
    ...
    node.on('mouseover', function (ev, d) {
      ...
      var grpDesc = d.group === 'LF' ? 'Berufsschulfach: theoretische Grundlage'
        : d.group === 'Comp' ? 'Fragenkomplex: bewertet die BlitzerSafe Analyse-Logik'
...
(function() {
  ...
  var cardObs = new IntersectionObserver(function(entries) {
    entries.forEach(function(entry) {
      ...
      // Slide BlitzerSafe card in from right (slight delay)
      if (lfa) {
        lfa.style.opacity    = '0';
